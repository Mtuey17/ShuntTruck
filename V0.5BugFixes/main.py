...
while 1:
    ...
    if currentTime>=startTime:
        ...
        if RelayCard.inputs[7]==2:#when there is a timeout from the relay card 
            ...
            turnSpeed,speedCount,toSendSpeed,forward,enable=calculateControls(RelayCard.inputs,feedBackSteeringPos,speedCount,previousDirection)
        ...
        if updatePrints==updateInterval:
            movmentMessages=True
            driveMotorMessages=False
            bmsMessages=False
            analogSensorMessages=False
            
            if movmentMessages:
                print("-------Movment Messages-------")
                print("controller error: %s" %controlErrorFlag)
                print("requested steering speed: %s" %turnSpeed)
                print("requested Speed: %s rpm" %toSendSpeed)
                print("Forward: %s" %forward)
                print("enable: %s" %enable)
                print(" ")
                #print("actualsteering position:    %s" %feedBackSteeringPos)
